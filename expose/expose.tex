...
This exposé\footnote{\url{http://www.softlang.org/info:expose}} outlines the thesis:3
\begin{center}
\it
\thesis
\end{center}
for acquiring the degree Bachelor Science (B.Sc.) in Computer Science.
The central topic of the thesis will be the study of traceability recovery in a megamodel governed environment, i.e. \megal, a modeling language for such models. Furthermore a system for exploring the recovered traceability links will be developed.
...
The motivating idea behind the thesis is that correspondence and conformance relationships between linguistic artifacts create trace-links left behind by transformations in the sense of \cite{DBLP:conf/sle/Lammel16}.
Thus, it should be possible to recover these links with a model describing such linguistic relations.
